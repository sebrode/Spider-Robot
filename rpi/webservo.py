    ...
    for j in range(loop_count):
        ...
        for i in range(max(matrix_full[0].shape[1], matrix_full[1].shape[1], matrix_full[2].shape[1], matrix_full[3].shape[1])):
            ...
            if i < matrix_full[2].shape[1]:
                redAngle1 = round(
                    90+math.degrees(float(matrix_full[2][0][i])), 2)
                redAngle2 = round(
                    90+math.degrees(float(matrix_full[2][1][i])), 2)
                redAngle3 = round(math.degrees(
                    float(abs(matrix_full[2][2][i]))), 2)

            if i < matrix_full[3].shape[1]:
                blueAngle1 = round(
                    90+math.degrees(float(matrix_full[3][0][i])), 2)
                blueAngle2 = round(
                    90+math.degrees(float(matrix_full[3][1][i])), 2)
